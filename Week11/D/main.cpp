...
//biginteger found here: https://github.com/Stupid-Dog/BigInteger/blob/stupid/BigInteger.cpp
#include <bits/stdc++.h>
...
struct Rational{
	...
	{
		BigInt b = gcd(denominator,numerator);
		numerator/=b;
		denominator/=b;
	}
	Rational(BigInt a, BigInt b): numerator(a), denominator(b) {};
};
int max_sides;
v parseString(string x, BigInt* pos)
{
	...
	*pos = Integer(1);
	...
	{
        vector<string> v = split(s[i], 'd');
        int dice = stoi(v[0]);
        int sides = stoi(v[1]);
        if(sides>max_sides)
			max_sides = sides;
		*pos*=pow(Integer(sides),Integer(dice));
...
Rational solve()
{
	...
	cin>>points>>x;
	BigInt pos;
	dice = parseString(x, &pos);
	BigInt mat[dice.size()+1][points+1];
	for(int i =0; i<=dice.size(); i++)
		for(int j=0; j<=points;j ++)
			mat[i][j] = Integer(0);
	for(int i =1; i<=dice.size(); i++)
		mat[i][0]=Integer((int)dice[i-1].size());
	mat[0][0] = Integer(1);
	for(int i = 1; i<= dice.size(); i++)
		for(int j = 1; j<= points; j++)
		{
			int val = max((int)dice[i-1].size() - j + 1, 0);
			mat[i][j] = Integer(val);
			BigInt p = Integer(1);
			for(int a = 0; a<i-1; a++)
				p*=Integer((int)dice[a].size());
			mat[i][j]*=p;
			for(int a = dice[i-1].size() - val; a>=1; a--)
				if(j-a >= 0) mat[i][j]+=mat[i-1][j-a];
		}
	Rational res = Rational(mat[dice.size()][points], pos);
	res.simplify();
	return res;
}
...
int main()
{
    ...
    for(int z = 1; z<=t; z++)
	{
		Rational sol = solve();
		cout<<"Case #"<<z<<": ";
		Print(sol.numerator); printf("/");
		Print(sol.denominator);EL;
	}
